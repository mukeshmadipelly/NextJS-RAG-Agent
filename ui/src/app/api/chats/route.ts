...
export async function GET() {
  try {
    const upstream = await fetch(`${BACKEND_URL}/chats`, { cache: "no-store" });
    const body = await upstream.text();
    return new Response(body, { status: upstream.status, headers: proxyHeaders(upstream) });
  } catch (error) {
    console.error("Fetch error in GET /api/chats:", error);
...
export async function POST(req: Request) {
  try {
    ...
    console.error("Fetch error in POST /api/chats:", error);
    return new Response(JSON.stringify({ error: "Failed to fetch from backend" }), {
      status: 502,
      headers: { "Content-Type": "application/json" },
    });
